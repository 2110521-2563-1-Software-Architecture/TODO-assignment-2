...
if (command == 'list')
    ...
    // yarn ts-node clientC.ts repeat list
    repeat(process.argv[0], process.argv[1]);
else if (command == 'repeat2')
    // yarn ts-node clientC.ts repeat2 list 4096
    repeat2(process.argv[0],process.argv[1]);
else if (command == 'fork')
    // yarn ts-node clientC.ts fork
    createClientProcess();